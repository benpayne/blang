# Epic: modules-v2 — Modules & Visibility: a real import graph and an opaque-type export model

**Archetype**: evolve (replaces the resolution and export model under the
existing module surface)

**Status**: draft — scoping complete, ready for `/devbot-plan`
...
Decision record below). Every "today" claim in this document was **reproduced
against the compiler** at `master` @ `7e75352`; the reproductions are quoted
inline so a reader can re-run them.
...
to its owner's layout beyond a pointer's width. That is what makes the export
model below possible without opaque handles, ARC changes, or codegen upheaval.
...
1. **Exact prelude membership.** `Map` and `Set` pass D13's bar comfortably and
   `Buffer` probably does, but the final list wants one deliberate call — it
   also sizes the migration, since today's code uses those names bare
   *because* they were force-promoted, so putting them in the prelude means
   that code doesn't change at all.
2. **The identity token for path dependencies.** `url@pin` is
   machine-independent; an absolute `realpath` is not, which matters for
   reproducible builds and a shareable cache.
