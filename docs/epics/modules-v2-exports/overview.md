...
|---|----------|----------|--------|--------|
| 1 | Canonical cross-module spelling for generated data-contract functions (e.g. `Todo_from_json`) under D1 — module-qualified name, or builtin dispatch like `to_json`? | U5 only | **IMPLEMENTED as the symmetric builtin (U5b, commit `c042fff`); surfaced to the owner at PR #3 review for the final call** | Type-directed `Todo.from_json(str)` symmetric with value-directed `to_json(value)` — both dispatch through the compiler, no bare mangled symbol (D1/D15, one canonical spelling). Reuses the already-registered `<Struct>_from_json` extern; bare `Todo_from_json(str)` still works (backward compatible). Lower-risk alternative (document the literal spelling, no code) is a trivial revert if the owner prefers it. |
