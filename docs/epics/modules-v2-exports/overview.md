...
|---|----------|----------|--------|--------|
| 1 | Canonical cross-module spelling for generated data-contract functions (e.g. `Todo_from_json`) under D1 — module-qualified name, or builtin dispatch like `to_json`? | U5 only | **RESOLVED (2026-08-09, product-owner ruling): Option A — the symmetric builtin `Todo.from_json(str)`** (U5b, commit `c042fff`). No revert. | Type-directed `Todo.from_json(str)`, symmetric with value-directed `to_json(value)` — both dispatch through the compiler, no bare mangled symbol (D1/D15, one canonical spelling). Reuses the already-registered `<Struct>_from_json` extern; the bare `Todo_from_json(str)` form stays as an additive, link-safe alias (backward compatible). The owner ruled Option A at PR #3 review. |
