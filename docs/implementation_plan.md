...
| 96 | Codegen for channel operations | impl | YES | `genChanMethodCall` emits `__blang_chan_send`/`__blang_chan_recv`/`__blang_chan_close`; element marshalled through a typed stack slot; tested in `codegen_channel.b` and `codegen_channel_spawn.b` |
