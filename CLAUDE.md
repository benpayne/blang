...
├── examples/                  # Standalone example programs (real programs used to shake out bugs)
│   ├── worker_pool/           # Concurrency example (spawn + channels): parallel prime counting
│   ├── todo_app/             # REST API + SQLite todo app (query/insert/@json/migrate)
│   └── calculator/           # Expression interpreter: tokenize -> recursive-descent eval over Result, with colocated `bcc test` suite
...
- **Channels** — `chan<T>` typed channels with `.send(value)`, `.close()`, and `.recv() -> Option<T>` (`some(value)` on success, `none` when closed and empty) operations (buffered, thread-safe; usable across `spawn` boundaries). Channels are reference-counted: the creating scope holds one reference and each capturing `spawn` retains its own, so the channel is freed only after both the creator and all capturing green threads finish (no leak, no use-after-free regardless of finish order).
...
- **Test blocks** — `test "name" { ... }` for built-in unit testing. `bcc test` compiles each `.b` file that defines `test` blocks into a runnable test binary (via `qcc --test-main`, which emits the test runner as `main` and skips the user's own `main`) and executes it. Tests run in isolation: a failed `assert` reports and the runner continues with the next test (setjmp/longjmp in the runtime) rather than aborting the whole process. Output is a per-test `PASS`/`FAIL` line (failures include the message + source line) and a `N passed, M failed` summary; the runner exits nonzero if any test fails.
- **Assert statements** — `assert expr;` and `assert expr, "message";`. Inside a `test` block a failed assert unwinds to the runner (per-test isolation); outside any test (normal code / `requires` / `ensures`) it prints the message + line and `exit(1)`s.
...
**CLI features**: `qcc` supports `--parse-only`, `-S`/`--emit-ir`, `-c`/`--emit-obj`, `-o`/`--output`, `--emit-bmod <file>`, `--combine` (compile multiple `.b` files into a single `.ll` with shared scope), `--help` flags and multiple input files (`.b` and `.bmod`). `bcc build` reads `blang.toml`, resolves dependencies, and builds projects (lib→.a+.bmod, bin→executable) with content-addressable caching. `bcc clean` removes the build cache. `bcc test` subcommand discovers `.b` files that define `test` blocks, compiles each into a test binary (`qcc --test-main` → llc → cc, linking the runtime), runs it, and aggregates per-file pass/fail results. `bcc migrate` subcommand supports `--preview`, `--apply`, `--generate`, and `--allow-destructive` modes for schema migrations: it runs `qcc --emit-schema` to extract the current table-struct schema, diffs it against `.blang/schema.json`, and (for `--apply`) executes the migration SQL against the `[database]`/`BLANG_DATABASE_URL` connection, then snapshots the new baseline. Destructive steps (DROP TABLE/COLUMN) are refused without `--allow-destructive`. `qcc` adds `--emit-schema <file>` and database-config flags (`--db-driver`/`--db-url`/`--db-conn`) forwarded by `bcc` from blang.toml.

**Runtime libraries**: `blang_string` (safe immutable string type — `BlangString` with refcounting, heap allocation, and bounds-checked access), `blang_array` (safe generic array type — `BlangArray` with refcounting, bounds-checked access, and dynamic growth), `blang_json` (C library for JSON encode/decode), `blang_net` (TCP sockets + poll-based Selector event loop — listen/accept/connect/read/write/close, event-driven I/O with fd-based handle table), `blang_db` (database abstraction with a driver-dispatch layer: tested SQLite backend + compile-guarded PostgreSQL/libpq backend, runtime SQL parameter binding, a process-wide default/named connection registry — `__blang_db_default`/`__blang_db_get` with `env:VAR` and `BLANG_DATABASE_URL` resolution — used by query/insert/update/delete codegen so a real connection and bound params reach the DB).

**Stdlib**: `stdlib/net.b` defines BLang-native types — `Socket`, `ServerSocket` (with methods via `impl FileOps`), `Selector` (with `on_accept`, `on_data`, `run`, `shutdown` methods using lambda callbacks), and a routed HTTP layer: `HttpServer` with a route table (`get`/`post`/`put`/`route(method, path, handler)` registration, `serve()` to parse+dispatch each connection, `on_request` for a single catch-all handler), `dispatch_request(routes, req)` (pure method+path matching → handler, else 404), `HttpRequest`/`HttpResponse`/`Route` structs, and BLang-native HTTP utility functions: `build_http_response()` (response building via string interpolation), `parse_http_request_line()` (Buffer-based request line parsing into `HttpRequestLine` struct), `parse_http_headers_from_buffer()` (Buffer-based header parsing into `HttpParsedHeaders` struct with parallel key/value arrays), `extract_http_body()` (extracts body after `\r\n\r\n` separator), HTTP client `http_get()`/`http_post()` (BLang-native Buffer I/O, return the response body), and `http_status_text()` (status code to reason phrase). (`delete` is a reserved keyword, so DELETE routes use `route("DELETE", ...)`.) These pure-BLang functions complement the C socket/selector runtime. Compiled into user programs via `qcc --combine stdlib/net.b user.b`; `bcc` resolves each `import <name>;` to the matching `stdlib/<name>.b` and combines only those modules (so an unused module never pollutes the namespace), while the corresponding runtime libraries are linked unconditionally. `stdlib/collections.b` defines `Map<K,V>` (key-value store using parallel arrays with `has`, `get`, `set` methods); `stdlib/timer.b` provides the timer/event-loop API (`every`/`after`/`cancel`/`run`/`stop`); `stdlib/sys.b` provides `args()`/`exit()`. SQL generation (`SQLGen`) converts query AST nodes to parameterized SQL. Schema migration (`SchemaMigration`) diffs table struct definitions against stored schema and generates CREATE/ALTER TABLE statements.

**Codegen features** (requires LLVM): function definitions, extern function declarations, variadic function calls, variable declarations with initialization, return statements, if/else (no parentheses), while (no parentheses), for-in loops, function calls, binary expressions (arithmetic, comparison, logical, bitwise with correct operator precedence and automatic integer type coercion for mixed-width operands; logical `&&`/`||` short-circuit — the right operand is generated in a separate block and evaluated only when the left does not already decide the result, so guards like `i < n && s[i] == c` never index out of bounds), function-call argument coercion (integer arguments are widened or narrowed to the parameter's LLVM integer type — e.g. a `true`/`false` literal, generated as i32, is narrowed to a `bool` (i1) parameter), assignment expressions (`=`, `+=`, `-=`, `*=`, `/=`, `%=`, `^=`), constant expressions (int, float, string, char), break/continue in all loop types (while, for, for-in, infinite), safe string type (`BlangString` — literals via `__blang_string_create_static`, concatenation via `__blang_string_concat`, comparison via `__blang_string_equals`, interpolation via `__blang_string_concat_many` with per-type to_string helpers, refcount tracking at scope boundaries), safe array type (`BlangArray` — literals via `__blang_array_create`/`__blang_array_push`, bounds-checked indexing via `__blang_array_get`/`__blang_array_set`, for-in iteration, refcount tracking at scope boundaries), `cstring`/`carray<T>` FFI types with automatic conversion at extern fn boundaries (string→cstring extracts `.data` field, cstring→string wraps via `__blang_string_create`), builtin method calls on string (`.length`, `.is_empty`, `.to_upper()`, `.to_lower()`, `.trim()`, `.contains()`, `.starts_with()`, `.ends_with()`, `.index_of()`, `.substring()`, `.replace()`) and Array (`.length`, `.is_empty`, `.push()`, `.pop()`, `.clear()`), pipeline operator (`|>` desugaring), ARC retain/release at scope boundaries for shared/sync variables, runtime init/shutdown in `main()` for concurrency features, spawn blocks with closure extraction (captured variables packed into context struct, dispatched to thread pool via `__blang_spawn`, `own` variables cannot be captured across spawn boundaries), channel variable declarations (`chan<T>` → `__blang_chan_create`) and channel operations (`.send(v)`/`.recv()`/`.close()` via `genChanMethodCall`; `.send`/`.close` call `__blang_chan_send`/`__blang_chan_close`, and `.recv()` returns the built-in `Option<T>` — receiving into the Option payload and setting the tag from the `__blang_chan_recv` success flag, so a closed+empty channel yields `none` and exhaustive match forces callers to handle it; channels are reference-counted — the creating body releases its reference at every exit path and each capturing `spawn` retains/releases its own, so a channel is destroyed only once both the creator and all capturing threads are done, eliminating the prior channel leak without risking use-after-free), async function wrappers (body extracted to `void*(void*)`, called via `__blang_async_call`), await expressions (`__blang_await` + `__blang_task_destroy`), event handler registration (`on EXPR { body }` extracts the body to a `void(ctx, fd)` callback and registers it on the global event loop via `__blang_event_on` keyed by the fd `EXPR` yields — a timerfd from `timer.every/after` or a socket fd — retaining refcounted captures for deferred invocation; a non-fd event expr falls back to inline invocation; registration does not fire the handler — the program enters the loop explicitly via `timer.run()`/`events.run()` (`__blang_event_run`), which blocks until `timer.stop()` or until no event sources remain), database query/insert/update/delete codegen (SQL generated at compile time via `SQLGen`, emits `__blang_db_query`/`__blang_db_exec` calls), enum/sum type tagged union layout (`{i32 tag, [N x i8] payload}` with variant construction and match-based payload extraction), exhaustive match checking on enums (a `match` on an enum that omits a variant without a wildcard `_` arm is a compile error naming the missing variant(s); non-enum subjects are unaffected), built-in generic `Option<T>` and `Result<T,E>` enums (registered in gScope + mEnumDefMap, available without user definition and shadowed by a user-defined `Option`/`Result`; type-erased 8-byte payload with the concrete type argument recovered at the match/`?` site from the subject's static type; match bindings receive their concrete QLang type so string `==`/methods work on bound payloads; a refcounted heap payload (string/Array/Buffer/struct) is given the enum its own reference at construction — strings transfer the temp, Array/Buffer/struct are retained — so a value like `Result.ok(a)` for a local array is not freed by its origin scope while the enum still references it, i.e. no use-after-free on unwrap), generic struct/function monomorphization (stamps out concrete versions like `Box_int` with mangled names), ownership move semantics enforcement (use-after-move and move-in-loop detection), `@json` annotation codegen (generates `StructName_to_json`/`StructName_from_json` functions supporting all primitive types, nested `@json` structs, with compile-time errors for unsupported field types) plus a builtin `to_json(value)` that resolves the argument's struct type at compile time and dispatches to `StructName_to_json` (compile error if the struct is not `@json`), enabling auto-serialized JSON responses via `net.http_json(to_json(x))`, `?` try operator codegen (resolves operand's enum type, extracts tag, branches on ok/some vs err/none, unwraps payload on success, propagates error via early return on failure), multi-module type sharing (`registerExternalTypes` shares struct/enum definitions across CodeGen instances for cross-module type resolution), combined compilation mode (`--combine` feeds multiple modules into a single CodeGen instance with shared scope, deduplicating extern/method declarations for stdlib integration), builtin `print`/`println` with compile-time format string validation (positional `{}` placeholders, format specifiers `{:x}`,`{:X}`,`{:o}`,`{:b}`,`{:.Nf}`,`{:e}`, Printable protocol for user-defined types, `@format` annotation for call-site validation on user-defined functions), and lambda/callback codegen (anonymous functions with closure capture via context struct + malloc, function types as `{fn_ptr, ctx_ptr}` pairs with parameter expansion, indirect calls through fn-typed variables, named function references via thunk wrappers with null context; a closure passed to an `extern` fn-typed parameter is retained an extra time so it survives the caller's scope and stays valid for deferred/cross-thread invocation, e.g. selector callbacks). Refcounted values bound to a local from an array element (`Point p = arr[i]`) are retained on bind — `__blang_array_get` borrows — matching the existing variable-copy and field-access retain so array-of-struct iteration does not double-free. The full pipeline (parse → LLVM IR → native binary) is tested end-to-end with 80 codegen E2E tests (including channel `Option<T>` recv single-threaded, across a spawn boundary, and the closed/empty `none` signal; exhaustive/wildcard enum matches; a match as the tail statement of `main()`; an `on` event handler registered from a helper function with the loop entered via `timer.run()` from `main`, exercising registration and loop-entry living in different functions; `bool` arguments passed to functions as both literals and variables; `&&`/`||` short-circuit evaluation; and an `Array` carried through `Result` and unwrapped).
...
- Memory management (ARC) is ASan-clean across the codegen E2E suite except for one known leak: an element removed from a **generic** collection via a discarded `.pop()` whose element type parameter resolves to a refcounted type (e.g. `Map<string, int>` removing a key) leaks that one element. The monomorphized `push(K key)` retains the element, but a discarded `pop()` on the same generic array orphans that reference; the non-generic `Array<string>` path is already balanced (so a blanket release-on-pop would double-free it). A correct fix needs monomorphization-aware refcounting at the pop site. All other previously-known leaks (DB string parameters, discarded `query` results, refcounted locals on `?` error paths, generic `Option`/`Result` payloads, struct-literal temporaries passed as direct or callback arguments) are fixed.
