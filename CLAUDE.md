...
- **Channels** — `chan<T>` typed channels with `.send(value)`, `.recv()`, and `.close()` operations (buffered, thread-safe; usable across `spawn` boundaries)
...
- Channel `recv()` returns the element value directly; the closed/empty signal (the `int` return of `__blang_chan_recv`) is not yet surfaced to BLang code as an `Option<T>`. A `recv()` on a closed, empty channel yields a zero-initialized value.
