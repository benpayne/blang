...
- **`cli` is an ordinary qualified library module** (modules-v2-graph U6, closes KG-6; completes done-condition 2's cli item + done-condition 3's cli special-case retirement). The former `isGlobalTypeLib` promotion of `cli` into the user scope (a codegen workaround for the module-prefix string-ARC double-free) is retired now that U2 fixed+regression-locked that path (F3 satisfied): `cli` falls through to the plain namespaced-stdlib routing (`qcc.cpp`), gets a `cli` module prefix like `net`/`fs`, and is accessed **qualified** — `cli.has_flag(...)`, `cli.flag_value(...)`, `cli.bool_flag(...)`, `cli.positionals(...)`. Its internal string-returning calls (`cli__has_flag → cli__flag_name_of`) are the exact prefixed shape U2's `nsarc` fixture proves clean, and `--leak-check` on `codegen_cli.b`/`codegen_flags_*.b` is **Leaks: 0** on the real module — the live proof **KI-3 is fully closed**. Call sites migrated (`codegen_cli.b`, `codegen_flags_edge.b`, `codegen_flags_parse.b`, `examples/kv/main.b`). The wider import-enforcement / flat-merge removal for `.bmod` deps (done-conditions 6/8) remains U6a/U6b.
- **Import enforcement for dependency functions + qualified-access codegen fork** (modules-v2-graph U6a, advances done-condition 6). A `.bmod` dependency's exported **free functions** no longer flat-merge into `gScope`: `injectBmodSymbols` (`qcc.cpp`) now registers a **per-module namespace scope keyed by the dep's import name on `gScope`** (the common ancestor of both the combine user scope and a non-combine module scope, so `mathlib.add` resolves in either build) and puts the functions there — **not** in `gScope`'s symbol list. So an **unqualified** `add(3,4)` no longer resolves (a **located** error at the call site; fixture `test_files/fail/xmodule/unqualified_import_call/`), and a dependency function is reachable only as `module.name` after `import module;` (D1) — resolution runs through the import graph. **Types (struct/enum/protocol) still land in `gScope`** as the deliberate U6a/U6b **bridge**: naming a foreign type (declaring a variable of it, constructing one) is D7 *name-capability* whose enforcement + the capability/collision diagnostics are **U6b**, so the type-using consumers (`Pair`/`Counter`/`Point`/`Box`/`Todo` construction, methods, `to_json`, `query`) stay green. The **codegen-naming fork** (`QExpression.cpp` qualified handler): a qualified call's emitted symbol is chosen by the callee's **provenance** — **generic** (either origin) → no mangled name, codegen monomorphizes via `mangleGenericName` (already prefix-free + U1-digest-keyed); **extern** (a `.bmod`-dep function, marked extern by `injectBmodSymbols`; also a real `extern fn`) → the **unprefixed** `memberName`, since the library `.a` exports plain `add` and codegen never prefixes an extern (`CodeGen.cpp:157`); otherwise a **combined-stdlib** function compiled in-process **with** the module prefix (`sys.args` → `sys__args`) → the prefixed name. Qualification is a pure source spelling with no ABI cost. **Corpus migrated** (the free-function consumers): `test_build/myapp` (`mathlib.add`/`multiply`/`largest`/`max3`), `test_build/boxapp` (`boxa.boxed_a`/`boxb.boxed_b`), `test_build/usebox` (`midx.get_box`), and the git-dep app (`greetlib.greeting`/`pick`) — those builds are the positive qualified-access proof. Remaining U6b: full type-injection removal + D7 name-capability model (positive+negative each direction) + KI-23 (DC9) + collision/import/unused-import diagnostics through the `DiagnosticEngine` with deterministic D3 display names (DC8). See `specs/039-enforce-imports-and-capability/`.
- **D7 use-vs-name capability model; the gScope type-injection bridge retired** (modules-v2-graph U6b-1, closes done-condition 6). `injectBmodSymbols` (`qcc.cpp`) now puts a dependency's exported **types (struct/enum) and protocols into its per-module namespace too — NOTHING lands in `gScope`** (the U6a function move + this type move together retire the flat merge; all name resolution runs through the import graph). The namespace is flagged `grantsNameCapability`. **Name-capability (D7):** the `import` handler (`QModule.cpp`) copies a **dependency** namespace's exported **type names** into the importing scope (`Scope::importTypeNamesFrom`, types + non-function symbols only, so functions stay qualified-only — the U6a enforcement holds), so `Pair<int> p`/`Counter(5)` resolve **only after** `import module;`; naming a foreign type without the import is a **located** error (`test_files/fail/xmodule/name_capability_requires_import/`). Combine-mode stdlib namespaces are **not** flagged, so `import net;` keeps `net.HttpServer` **qualified** (never dumped into the user scope — that regression surfaced on `codegen_http.b` and gated the fix). **Use-capability (D7):** holding a foreign value + calling its `pub` methods needs **no import** — it rides on CodeGen's `mStructDefMap`/`mEnumDefMap` (populated by `registerExternalTypes`), never on the source scope. The un-named foreign generic (`test_build/usebox` imports midx ONLY, never boxq) holds a `Box<int>` via **`var` inference** — U6b-1 fixed the pre-existing `var`-inference gap in `Sema` (a `var` bound to a struct/generic/string value was mistyped `i32`, dropping the method call): Sema now sets the `var`'s type from the initializer's resolved type, which is what makes use-capability real (hold + dispatch without ever naming the type). The `.bmod` **foreign-type pre-scan** (`qcc.cpp`) registers its placeholder in the **defining `.bmod`'s own fileScope**, not `gScope`, so an interface's foreign reference can't leak name-capability into a consumer. `BmodEmitter::emit` reads the **emitting module's** scope (not `gScope`) to resolve a foreign type for its `// foreign-type:` header. D7 fixtures both directions: name-capability positive (`test_build/counterapp`; explicit `Box<int>` with `import boxq`) + negative (`fail/xmodule/name_capability_requires_import`, transitive `run_build_tests.sh` D7 checks); use-capability positive (`usebox`) + use-does-not-grant-name negative (`run_build_tests.sh`). Remaining U6b-2: KI-23 (DC9) + collision/import/unused-import diagnostics + D3 display-name rendering + sharpening the located message to "did you mean `module.name`?" (DC8). See `specs/039-enforce-imports-and-capability/`.
- **Collision & import diagnostics; D3 "did you mean" sharpening** (modules-v2-graph U6b-2, closes done-condition 8). A whole-program import-diagnostics pass (`qcc.cpp`, after the parse loop) runs through the `DiagnosticEngine`, gated on an **authoritative** module set (combine mode, or dependency `.bmod`s on the line — a bare single-file parse like `import std;` stays lenient): an **unknown-module import** (`import nonesuch;`) is a **located error** (`fail/xmodule/unknown_module_import/`); an **unused import** is a **located warning** (`unused-import` slug, `-Werror` promotes). **Duplicate exported names** across two imported modules (P2): importing both is legal (D4 — two libs may each export `Box`; `test_build/boxapp` uses both via qualified functions and never names `Box`), so a collision is **not** an import-time error — `Scope::importTypeNamesFrom` **unbinds** the ambiguous name (so it can't silently bind to whichever was imported first, a P10 trap) and records it; a **bare use** of it is a located error naming both exporters (`QType.cpp`). **D3 "did you mean" sharpening:** an unqualified dependency function (`greet(...)` for `lib.greet`) now reports "undefined symbol 'greet' — did you mean 'lib.greet'?" (`QExpression.cpp` `VariableExpression::Parse` + `Scope::moduleExporting`), and an unimported foreign type reports "unknown type 'Widget' — did you mean to `import lib;`?" (`QVariableDefinition.cpp`, which also **fixed the pre-existing `Failed parse varible` typo** → "unknown type '…'" at the type token / "expected a variable name after type '…'"). Usage for the unused-import lint is marked at the qualified-access sites (`QExpression`/`QType`) and at bare imported-type resolution (`markTypeUsed`). Goldens updated: `fail/sema/{undefined_function,undefined_variable,buffer_without_prelude}` + `fail/undefined_var` `.expected`, LSP `diag_open`/`diag_change` transcripts (message text only). DC8 fixtures: `fail/xmodule/{unqualified_import_call,unknown_module_import}` + `run_build_tests.sh` DC8 checks (unused-import warn + `-Werror`, ambiguous bare use). **Remaining U6b-3:** KI-23 (DC9) — combine-mode field privacy Sema-enforced by U1 module identity. See `specs/039-enforce-imports-and-capability/`.
- **Combine-mode field privacy keyed on module identity (KI-23 closed)** (modules-v2-graph U6b-3, closes done-condition 9). Field-access privacy in `Sema` (`resolveFieldAccess`) previously keyed on `StructDefinition::isFromInterface()` — the `.bmod`-arrival heuristic, which **missed** the combine-mode namespaced-stdlib case (a user reaching into a `Map`/`Set`/`net`/`fs` struct's private field, caught only by the `check_no_field_reachins.sh` grep gate over `examples/`/`test_build/`, never in `test_files/`). Now it keys on **module identity**: a field access on a struct whose **defining module** (the source-file basename, stamped by `stampDefiningOrigin` on both the struct AND the `Module`, `getDefiningFile()`) differs from the **use-site** module (`Sema::mModuleId`, the module being analyzed) is private — `imported = isFromInterface() || crossModule`. One rule for every module boundary, enforced in **all build modes** (`Sema` runs everywhere). Uses the file basename, not the U1 digest, because in a `bcc` combine build every `.b` shares the project's `--module-origin` digest (so the digest can't separate `collections` from the user), whereas `stampDefiningOrigin` gives each file its own basename. `Sema::analyze` already received the `Module*` (no signature change), so `lsp/Compile.cpp` routes through the same path unchanged (blangd is single-file → both ids empty → `crossModule` false → DC9 inert, `test_lsp` green). The one white-box reach-in in the corpus (`codegen_map_hashed.b`'s `m.buckets.length`) was migrated to a new `pub fn Map.bucket_count()` accessor (`stdlib/collections.b`) — the reach-in grep gate is no longer the only guard. Fixtures (`run_build_tests.sh` DC9 checks): a combine-mode `m.buckets` reach-in is a **located** field-privacy error (negative), and the same observation via `m.bucket_count()` compiles clean (positive). See `specs/039-enforce-imports-and-capability/`.
- **Import aliasing `import x as y;`** (modules-v2-graph U8, closes done-condition 10, stretch). A dependency or module can be bound to a **local qualifier**: `import mathlib as m;` makes `m.add(...)` the qualified access path and grants D7 name-capability under the alias (`Pair<int>` is namable). `as` is a **contextual keyword** (lexes as a plain SYMBOL, `QModule.cpp`), so no lexer/keyword change. The `ImportStatement` carries the real module + the alias; the import handler registers the module's real namespace **also under the alias** (`addNamespace(y, ns)`) and records `y→x` (`Scope::addModuleAlias`). The alias is a pure source **rebind** (D8: no re-export / dual binding) — after `import x as y;` the original name `x` is **not** bound, so `x.foo` is a located error (`fail/xmodule/alias_rebind/`, both build modes). The **codegen-prefix fork** (`QExpression.cpp`) resolves the alias to the real module for a **combined-stdlib** callee (`import sys as s; s.args()` emits `@sys__args`, not `@s__args`, via `Scope::realModuleName`); a `.bmod`-dep callee is extern → unprefixed, so it is alias-independent and links from the library `.a`. Diagnostics/usage key on the **local qualifier** (`ImportStatement::getLocalQualifier()`): the unused-import lint checks the alias; the unknown-module check verifies the real module. Positive proof: `test_build/aliasapp` (`import mathlib as m;` builds, links, runs — `m.add`/`m.multiply`/`m.largest` link from `mathlib.a` + `Pair<int>` name-capability). See `specs/040-import-aliasing/`.
- **Foreign type references in `.bmod` + a transitive build graph** (modules-v2-graph U5, done-condition 7, closes KI-5). A `pub` signature that returns/takes a type **owned by another module** (a lib imports `Q` and exposes `Q`'s `Box<int>`) emits a `// foreign-type: <name> <module> <12-hex-digest>` header in its `.bmod` — the foreign type **by U1 identity digest** + human name, **not** an embedded body (`BmodEmitter::emit` takes the scope; format bumped **4→5**, salt reaches `BuildCache::computeKey`, proven by the `build_cache_key` ctest). The reader **pre-scans** those headers and registers the type name **before** parsing, so the interface parses **standalone** (`bmod_parses`) even without the defining `.bmod` on the line; a malformed foreign-ref is a **located error, not a crash** (Quality Gate 7). `bcc` assembles the **transitive `.bmod`/`.a` closure** in topological order (`buildProject` propagates a deduped `DepArtifact` closure; a dependency precedes its dependent) and `qcc` injects `.bmod` symbols **incrementally** per-parse — so a consumer that imports `X` (which imports `Q`) receives `Q`'s interface/archive and can **monomorphize a foreign generic `X` returns from `Q` without ever naming `Q`** (D7 use-capability across an un-named module). The instance mangles with `Q`'s carried digest → `linkonce_odr` dedup with the lib's own instance (the U1/U5 interlock). Proven by `test_build/boxq`+`midx`+`usebox` (usebox imports midx only, runs; `nm` shows one shared digest) and a malformed-`.bmod` security check. See `specs/038-bmod-foreign-refs-transitive-graph/`.
- **Resolution runs through a standalone `Resolver` component** (modules-v2-graph U4, the Epic C seam). Resolution used to be built inline in `qcc.cpp main()` (and separately in `lsp/Compile.cpp`). `Resolver` (`Resolver.h`/`Resolver.cpp`, in `BLANG_FRONTEND_SOURCES`) owns the global builtin scope (a `SmartPtr<Scope>`), installs/restores the `gScope` alias across construct/destruct, and hands out module scopes (`newModuleScope`) plus a namespace registry. **Both `qcc.cpp` (`Resolver resolver;`) and `lsp/Compile.cpp` (`static Resolver sResolver;`) construct it** (grep-verifiable) — the single shared entry point Epic C consumes to wire cross-module editor features without re-plumbing. U4 is **behavior-neutral**: the `.bmod` flat-merge injection and the combine routing policy stay in the driver (their removal is U6), and `blangd` stays single-file (it constructs the resolver but calls only `globalScope()`/`newModuleScope()`; the module-graph methods are present-but-dormant until Epic C). Two ctests: `resolver_component` (exercises the resolver independent of any driver) and `resolver_reuse` (builds the resolver as `lsp/Compile.cpp` does + calls the real `lsp::compileDocument`, asserting a fixture resolves identically to the qcc path). See `specs/037-module-graph-and-resolver/`.
- **Three type tiers: core / prelude / library** (modules-v2-graph U3, D12/D13/D14). The hardcoded `isGlobalTypeLib` promotion list is replaced by a declared, closed **prelude manifest** — exactly `{Map, Set, Buffer}` (types only; `isPreludeTypeName`, `Frontend.h`/`QModule.cpp`). Prelude types are **automatically in scope with zero imports**: their modules (`buffer.b`, `collections.b`) are **unconditionally loaded** (bcc + `test_codegen.sh`) but LOADED into a prefix-free **holding namespace scope**, then a per-module **PROMOTE** injects only the prelude *types* into a dedicated **prelude scope** that PARENTS the user's combine scope — so a user's own `struct Map` **shadows** the prelude uniformly by child-first lookup (both scope registries), with an `allStructs` shadow filter reconciling CodeGen's `mStructDefMap` (the third registry). This LOAD-vs-PROMOTE split keeps prelude symbols out of the user scope at parse time (no split-brain). `collections` is a **mixed module**: `Map`/`Set` → prelude; the free function `sort` → qualified `collections.sort(...)` (library tier; bare `sort(` call sites migrated). The bare-name `addType("Buffer")` is **deleted** (D14): `Buffer` resolves from `stdlib/buffer.b`, and absent the prelude a `Buffer` type fails at the declaration (`fail/sema/buffer_without_prelude.b`). `Task`/`Array` stay registered as **core**. (**cli was demoted to an ordinary qualified library module in U6** — see the cli-demotion entry below.) Fixtures: `codegen_prelude_no_import.b` (zero-import Map/Set/Buffer), `codegen_prelude_shadow.b` (user Map with a distinct API shadows the prelude), and the three self-`Map` tests (`codegen_map.b`, `codegen_arc_map_struct_value.b`, `codegen_ix_method_chain_field.b`) as the shadowing regression guard. See `specs/036-type-tiers/`.
