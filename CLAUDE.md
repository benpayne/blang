...
- Channel `recv()` returns `Option<T>` (`some(value)` on success, `none` when the channel is closed and empty). The `Option` is a concrete per-element-type enum synthesized at codegen time (`Option_<T>`); it is not yet unified with a user-defined or built-in generic `Option<T>`, so its compiler-internal name (e.g. `Option_int`) appears in non-exhaustive-match error messages.
