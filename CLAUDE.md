...
- **Event handlers** — `on expr { ... }` registers the body on the global event loop keyed by the fd `expr` yields (e.g. `on timer.every(1000) { ... }`); `timer.run()`/`timer.stop()` drive/stop the loop (timer + socket fds share one poll loop)
...
- Event handlers (`on EXPR { }`) register on the global poll-based event loop when EXPR yields an fd (timerfd/socket fd); a non-fd event expression still falls back to inline invocation. The HTTP server's Selector is a separate loop instance (not yet unified with the global `on` loop).
