...
- Query result sets are not yet mapped back to BLang values (row→struct / `Array<T>` / `first` → `Option<T>`); `query`/`insert`/`update`/`delete` execute and `insert`/`update`/`delete` return affected-row counts, but reading SELECT columns from BLang requires extern calls to `__blang_db_result_*`. Row-to-struct mapping is the next data-layer feature.
