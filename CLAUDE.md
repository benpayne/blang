...
- **Event handlers** — `on expr { ... }` registers the body on the global event loop keyed by the fd `expr` yields (e.g. `on timer.every(1000) { ... }`). The loop runs automatically when `main` falls through (no `timer.run()` needed) and exits when stopped (`timer.stop()`) or when no event sources remain. `timer.after(ms)` (one-shot), `timer.cancel(source)` (remove one timer), and an explicit `timer.run()` are also available. Timer + socket fds share one poll loop.
...
- Event handlers (`on EXPR { }`) register on the global poll-based event loop when EXPR yields an fd (timerfd/socket fd); the loop auto-runs at the end of `main` (if it falls through) and exits on `timer.stop()` or when no sources remain. A non-fd event expression still falls back to inline invocation. The HTTP server's Selector is a separate loop instance (not yet unified with the global `on` loop). Auto-run only fires when `main` falls through; a `main` that ends with an explicit `return` after registering handlers should call `timer.run()` explicitly.
